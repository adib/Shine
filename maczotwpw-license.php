...
$abbreviations_list = array('WPW');

$app = new Application();
$app->select('WPW', 'abbreviation'); // custom
if (!$app->ok()) {
	error_log("Application WPW not found!");
	die("Application WPW not found!");
}

$lt = new LicenseType();
$lt->select('WPW_MACZOT_01', 'abbreviation'); // custom
if (!$lt->ok()) {
	error_log("License type WPW_MACZOT_01 not found!");
	die("License type WPW_MACZOT_01 not found!");
}
